    ...
    <el-table v-loading="loading" :data="postList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="简介" align="center" prop="intro">
        <template #default="scope">
          <span>{{ scope.row.intro.slice(0, 30) }}</span>
          <span v-if="scope.row.intro.length > 30">...</span>
        </template>
      </el-table-column>
...
import {listPost, getPost, delPost, addPost, updatePost} from "@/api/oj/post";
...
function handleDelete(row) {
  ...
  modal.confirm('是否确认删除文章编号为"' + _ids + '"的数据项？').then(function () {
    return delPost(_ids);
  }).then(() => {
    getList();
