<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="roomList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="用户1" align="center" prop="roomFriend.user1">
        <template #default="scope">
          <div style="display: flex; flex-direction: column; align-items: center"
               :title="scope.row.roomFriend.user1.userName">
            <img :src="scope.row.roomFriend.user1.userAvatar" style="width: 25px; height: 25px; border-radius: 50%">
            <span>{{ scope.row.roomFriend.user1.userName.slice(0, 5) }}<span
                v-if="scope.row.roomFriend.user1.userName.length > 5">...</span></span>
          </div>
        </template>
      </el-table-column>
      <el-table-column label="用户2" align="center" prop="roomFriend.user2">
        <template #default="scope">
          <div style="display: flex; flex-direction: column; align-items: center"
               :title="scope.row.roomFriend.user2.userName">
            <img :src="scope.row.roomFriend.user2.userAvatar" style="width: 25px; height: 25px; border-radius: 50%">
            <span>{{ scope.row.roomFriend.user2.userName.slice(0, 5) }}<span
                v-if="scope.row.roomFriend.user2.userName.length > 5">...</span></span>
          </div>
        </template>
      </el-table-column>
      <el-table-column label="活跃时间" align="center" prop="activeTime" width="180">
        <template #default="scope">
...
import {listRoom, getRoom, delRoom, addRoom, updateRoom, listRoomFriend} from "@/api/oj/room";
...
function getList() {
  ...
  listRoomFriend(queryParams.value).then(response => {
